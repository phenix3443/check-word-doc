        ...
        for match in matches:
            ...
            if between_text.strip() == "" and len(between_text) > 0:
                ...
                if re.match(verify_pattern, matched_text):
                    ...
                    if (
                        text_elem1 is not None
                        and text_elem2 is not None
                        and text_elem1 == text_elem2
                    ):
                        ...
                        issues.append(
                            {
                                ...
                                "page": estimate_page_from_paragraph(
                                    para_idx, total_paragraphs
                                ),
                                "text": full_match,
                                "position": start_pos,
                                "context": para_text[
                                    max(0, start_pos - 20) : end_pos + 20
                                ],
                            }
                        )
                        continue
                            ...
                            for run in all_runs:
                                ...
                                if found_elem1:
                                    ...
                                    if run.find(".//m:oMath", namespaces) is not None:
                                        ...
                                        break
                                    if (
                                        run.find(".//m:oMathPara", namespaces)
                                        is not None
                                    ):
                                        content_type = "formula"
                                        break
        ...
        with zipfile.ZipFile(docx_path, "r") as docx:
            ...
            for para_idx, para in enumerate(paragraphs, 1):
                ...
                if not para_text.strip() and para_idx <= 30:
                    ...
                    if (
                        pPr is not None
                        and pPr.find(".//w:sectPr", namespaces) is not None
                    ):
                        continue
            ...
            if len(para_info_list) > 50:
                with ThreadPoolExecutor(max_workers=8) as executor:
                    future_to_para = {
                        executor.submit(_check_paragraph_spacing, para_info): para_info[
                            0
                        ]
                    ...
                    for future in as_completed(future_to_para):
                        try:
                            ...
                            print(
                                f"   Warning: Error processing paragraph {future_to_para[future]}: {e}"
                            )
            else:
                # 对于小文档，串行处理更快（避免线程开销）
                for para_info in para_info_list:
        ...
        if text_elem in list(run.iter()):
            ...
            if run_container_index >= 0:
                ...
                if run_container_index > 0:
                    ...
                    if prev_elem.find(
                        ".//m:oMath", namespaces
                    ) is not None or "oMath" in str(prev_elem.tag):
                        return True
                ...
                if run_container_index < len(para_children) - 1:
                    ...
                    if next_elem.find(
                        ".//m:oMath", namespaces
                    ) is not None or "oMath" in str(next_elem.tag):
                        return True

    return False


def check_chinese_quotes(
    docx_path,
    check_english_quotes=True,
    check_quote_matching=True,
        ...
        with zipfile.ZipFile(docx_path, "r") as docx:
            if "word/document.xml" not in docx.namelist():
                return {
                    "found": False,
                    "message": "Document body not found",
                    "details": {"english_quotes": [], "quote_matching": []},
                }
                    ...
                    for match in matches_double:
                        ...
                        for tp in text_positions:
                            if match.start() < tp["end"] and match.end() > tp["start"]:
                                if is_text_in_formula_context(
                                    tp["element"], para, namespaces
                                ):
                                    in_formula = True
                                    break
